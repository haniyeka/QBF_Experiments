...
ReadQ::ReadQ(Reader& r, bool _qube_input)
    :
    r(r), qube_input(_qube_input), qube_output(-1), max_id(0), _header_read(false)
{}
...
bool ReadQ::get_header_read() const
{ return _header_read; }

Var ReadQ::get_max_id() const
{ return max_id; }

vector<Quantification>& ReadQ::get_prefix()
{ return quantifications; }

const vector<vector<Lit>>& ReadQ::get_clauses() const
...
void ReadQ::read_cnf_clause(Reader& in, vector<Lit>& lits)
{
  ...
  for (;;)
  {
    ...
    lits.push_back(parsed_lit > 0 ? mkLit(v) : mkLit(v, true));
  }
}
...
void ReadQ::read_quantification(Reader& in, Quantification& quantification)
{
  ...
  quantification.second = variables;
}
...
void ReadQ::read_clauses()
{
  ...
  for (;;)
  {
    ...
    // sort the read clause
    sort(ls.begin(), ls.end());
    
    // eliminate any double entries
    if (!ls.empty())
    {
      Lit current = ls[0];
      unsigned pos = 1;
...
void ReadQ::read()
{
  ...
  vector<Var> toadd;
  unquantified_variables.convert(toadd);
  
  if (!toadd.empty())
  {
    if (!quantifications.empty() && quantifications[0].first == EXISTENTIAL)
    {
      // if the first layer is existential, add them there
      quantifications[0].second.insert(quantifications[0].second.end(), toadd.begin(), toadd.end());
    }
    else
    {
      // create a new existential layer in front and add them there
      Quantification quant(EXISTENTIAL, toadd);
      quantifications.insert(quantifications.begin(), quant);
    }
  }
}
