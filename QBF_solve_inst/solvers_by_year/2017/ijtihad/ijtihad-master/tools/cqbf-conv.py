...
##############################################################################
# cqbf-conv.py: Converts between QDIMACS and the GhostQ CQBF format.
# See qpro-to-cqbf.py for converting from the QPRO format.
# Authors: Will Klieber
#          Samir Sapra (Unprenexing) 
##############################################################################
...
class ProbInst(object):
    ...
    def IsGateLit(self, x):
        ...
        #ret = (self.LevByVar[abs(x)] > 1)
        ##if (not glo.NoRenum): assert(ret == (abs(x) > self.NumVars))
        #return ret
...
def read_input_file(filename):
    ...
    if (FirstLine.startswith(("c")) or FirstLine.startswith("p cnf")):
        if glo.TryBloqqer:
            if filename == '-':
                die("Must read from a file (not stdin) to use bloqqer.")
            from tempfile import mkstemp, TemporaryFile, NamedTemporaryFile
            import subprocess
            bloqqer_exe = glo.TryBloqqer
            bloq_out = NamedTemporaryFile()
            [NumVarsOrig, NumClausesOrig] = read_qdimacs_probline(in_file)
            if (PrStage): 
                sys.stderr.write("Original: %6i vars,  %7i clauses.\n" % (NumVarsOrig, NumClausesOrig))
                sys.stderr.write("Bloqqer:  ")
            subprocess.call([bloqqer_exe, filename, bloq_out.name])
            bloq_out.seek(0)
            bloq_reader = LineReader(bloq_out)
            [NumVarsBloq, NumClausesBloq] = read_qdimacs_probline(bloq_reader)
            if (PrStage): 
                sys.stderr.write("%6i vars,  %7i clauses.\n" % (NumVarsBloq, NumClausesBloq))
            if NumVarsBloq <= 18:
                in_file.close()
                in_file = bloq_reader
            else:
    ...
    for prefix in ["#", "LastInputVar", "CktQBF"]:
        if FirstLine.lower().startswith(prefix.lower()):
            return read_circuit_file(in_file)

    die("Unrecognized file format.")


def read_circuit_file(in_file):
    def LineDie(msg):
...
#class TestBddSet(object):
#    
#    rebal_thres = 200000
#
#    def __init__(self):
#        self.set = set()
#        self.bdd_set = None #BddSet()
#
#    def __contains__(self, item):
#        if self.bdd_set is not None:
#            ret = (item in self.bdd_set)
#        else:
...
#            assert(0)
#        #if self.bdd_set is not None:
#        #    self.bdd_set = self.bdd_set | BddSet([item])
#
#    def __ior__(self, other):
#        if (self.set is not None) and (other.set is not None):
#            ret = self.set | other.set
#            if len(ret) <= self.rebal_thres:
#                self.set = ret
#                return self
#        self.bdd_set = self.bdd_set if self.bdd_set else BddSet(self.set)
#        other.bdd_set = other.bdd_set if other.bdd_set else BddSet(other.set)
#        self.bdd_set = self.bdd_set | other.bdd_set
#        self.set = None
#        return self
#
#    def rebalance(self):
#        if len(self.set) > self.rebal_thres:
#            self.bdd_set = BddSet(self.set)
#            self.set = None
        

            

#ret = FrozRadixSet.null
...
def cnf_to_nnf(UsedVars, NumVars, QuantBlks, Clauses):
    ...
    #         assert(len(val) > 1)
    #         assert(abs(key) not in DbgInelig)
    #         dict.__setitem__(self, key, val)
    #         for x in val[1:]:
    #             DbgInelig.setdefault(abs(x), []).append(key)
    # RepTable = DebugDict()
    ...
    if 1 and len(QuantBlks) == 3 and QuantBlks[1].qtype.lower() == 'a' and (4 <= len(QuantBlks[1].qvars) <= 6):
        ...
        if 0:
            for CurCl in Clauses:
                if len(CurCl) != 6: continue
                elits = []
                ulits = []
                for lit in CurCl:
                    if abs(lit) in UnivVars:
                        ulits.append(lit)
                    else:
                        elits.append(lit)
                if len(elits) != 2: continue
                if   QbFromVar[abs(elits[0])] == 0 and QbFromVar[abs(elits[1])] == 2: ixOuter = 0
                elif QbFromVar[abs(elits[1])] == 0 and QbFromVar[abs(elits[0])] == 2: ixOuter = 1
                else: continue
                (outer, inner) = (elits[ixOuter], elits[1 - ixOuter])
                ImpDefs.setdefault(inner, set())
                ImpDefs[inner].add(tuple(sorted([-x for x in [outer] + ulits], key=abs)))
        else:
            for CurCl in Clauses:
        ...
        #stderr.write("NARF[%i]: " % len(ImpDefs) + str(VarsCovered) + "\n")
        del ImpDefs
    ...
    if QuantBlks == []:
        ...
        if (inr.qtype.lower() == 'e'):
            ...
            assert(inr.qtype.lower() == 'a')

    DelClauses = set()

    def SubAli(x):
        while True:
            ret = prob.VarAliases.get(x, None)
            if ret == None:
                return x
            x = ret
        assert(ret not in prob.VarAliases)
        return ret
    ...
    glo.xsub = xsub  #DEBUG!, TODO: delete this
    ...
    if len(Clauses) > 100000 and NumOrigVars > 20000:
        glo.NoMonoRep = True
            ...
            if len(CurCl) > 2:
                for RepLit in CurCl:
                    ...
                    if len(TestCl) == 2:
                        ...
                        break
            if len(ModCl) == 2:
                #paired[ModCl[0]][ModCl[1]] = True
                #paired[ModCl[1]][ModCl[0]] = True
                #paired[ModCl[0]].add(ModCl[1])
                #paired[ModCl[1]].add(ModCl[0])
    ...
    NewClauses = list(reversed([tuple(SubAli(x) for x in clause) for clause in Clauses if (clause not in DelClauses)]))

    def ConvRepPair(key, val):
        NewKey = SubAli(key)
        NewVal = tuple(SubAli(x) for x in val)
        if NewKey < 0:
            NewKey = -NewKey
...
def RenumExprs(p):
    ...
    if (len(FreeVars) > 0): 
        print "There are unquantified variables in the QBF matrix:"
        print FreeVars
        stop()
        qdie("")
    #
...
def GatesSorted(p, dir, MinLev=2):
    for (lev, vars) in sorted(p.VarsByLev.items(), key=(lambda (lev,v): lev*dir)):
        if (lev < MinLev):
            continue
        for v in vars:
            yield v

def Gates_BotUp(p, MinLev=2):
    for x in GatesSorted(p, 1, MinLev):
        yield x

def Unprenex(p):
    if not glo.TryUnprenex: return
    p.NumPushed = 0
    dp = 0  # debug print
    assert(type(p) == ProbInst)
    FindParents(p)
    if dp: print p.VarsByLev
    if dp: print "\n"

    def getGateDesc(g):
        if g not in GateDesc:
            if p.LevByVar[g] == 1:
                GateDesc[g] = set([g]);
            else:
                GateDesc[g] = set()
                for gi in p.RepTable[g][1:]:
                    GateDesc[g] |= getGateDesc(abs(gi))
        return GateDesc[g]
    GateDesc = {}
    if (p.OutGlit < 0):
        return
    getGateDesc(p.OutGlit) # p.OutGlit must be >0
    #print GateDesc;

    for (lev, gates) in sorted(p.VarsByLev.items()):
        if (lev < 3): continue  # Skip input vars and lowest-level gates.
        gates = copy(gates)
        gates &= set(p.QuantPfx.keys())  # Set-intersect to exclude non-quantified gates
        for g in gates:
            for QB_idx in xrange(len(p.QuantPfx[g])-1, -1, -1):
                QB = p.QuantPfx[g][QB_idx]
                if dp: print g,",",QB_idx,": ", QB
                if dp: print "(Type %s %s)" % (QB.qtype, QB.qtype.lower())
                x_qtype = QB.qtype.lower()
                for var_idx in xrange(len(QB.qvars)-1, -1, -1):
                    x = QB.qvars[var_idx]
                    if dp: print "Var %d:" % x
                    g_new = g
                    pos = QB_idx
                    while 1:
                        if dp: print "        Gate %d, %d (%s) of %d" \
                              % (g_new, pos, \
                              len(p.QuantPfx[g_new]) and \
                                 p.QuantPfx[g_new][pos].qtype, \
                              len(p.QuantPfx[g_new]) - 1)
                        if pos < len(p.QuantPfx[g_new]) - 1:
                            if dp: print "        New QType: %s" % \
                                  p.QuantPfx[g_new][pos+1].qtype
                            if p.QuantPfx[g_new][pos+1].qtype.lower() \
                                   == x_qtype:
                                pos += 1
                            else:
                                break # stop: we hit a complementary
                                      # quantifier
                        else: # here pos == len(p.QuantPfx[g_new]) - 1
                            subscopes = [child \
                                         for child in p.RepTable[g_new][1:] \
                                         if x in GateDesc[abs(child)]]
                            if dp: print "        Subscopes: ", subscopes
                            if len(subscopes) == 1 and \
                                   subscopes[0] > 0 and \
                                   p.LevByVar[subscopes[0]] > 1:
                                g_new = subscopes[0]
                                p.QuantPfx.setdefault(g_new, [])
                                pos = -1
                            else:
                                break # stop: we've reached all the
                                      # way down to the immediate dominator
                    # Time to delete and re-insert
                    if (g_new, pos) != (g, QB_idx):
                        p.NumPushed += 1
                        if dp: print "Moving %d from (%d,%d) to (%d, %d)" % \
                              (x, g, QB_idx, g_new, pos)
                        del QB.qvars[var_idx]
                        if pos == -1:
                            p.QuantPfx[g_new].insert(0, QuantBlk(x_qtype, [x]))
                        else:
                            p.QuantPfx[g_new][pos].qvars.append(x)
                if len(QB.qvars) == 0:
                    del p.QuantPfx[g][QB_idx]
                if dp: print
            pass
            # TODO: Try to push quantifiers inward
    ...
    BIG_MAGIC_NUM=16
    for (lev, gates) in sorted(p.VarsByLev.items()):
        if (lev < 2): continue  # Skip input vars
        #for g in gates:
        if dp: print "Level %d: " % (lev),
        if dp: print sorted([len(GateDesc[g]) for g in gates])
        for g in gates:
            # 'continue's
            if g not in p.QuantPfx or len(p.QuantPfx[g]) == 0: continue
            if len(p.RepTable[g]) < BIG_MAGIC_NUM: continue
            
            if dp: print "Level %d, Gate %d [%d]: " % (lev, g, len(GateDesc[g])),
            if dp: print sorted([len(GateDesc[abs(child)]) for child in p.RepTable[g][1:]])
            ...
            VarsByLen[4] = set()
            VarsByLen[5] = set()
            for ch in p.RepTable[g][1:]:
                child = abs(ch)

                # Potential optimization: Figure out how to soundly
                # ignore large child gates

                for var in GateDesc[child]:
                    if var not in p.QuantPfx[g][-1].qvars: continue
                    #   alternate way of iterating
                    
                    Vars2gates.setdefault(var,[])
                    Vars2gates[var].append(child) #get too big?
                    newvarlen = len(Vars2gates[var])
                    if newvarlen == 1:
                        VarsByLen[newvarlen].add(var)
                    elif newvarlen <= SML_MAGIC_NUM+1: #5:
                        VarsByLen[newvarlen-1].remove(var)
                        VarsByLen[newvarlen].add(var)
            if dp: print "Potential Inverse Map:", p.QuantPfx[g][-1].qvars
            if dp: print "Invserse Map:", VarsByLen
            if dp: print "Invserse Map (contd):", Vars2gates

            for varlen in xrange(SML_MAGIC_NUM, 1, -1): #[4, 3, 2]:
                while len(VarsByLen[varlen]) > 0:
                    var = iter(VarsByLen[varlen]).next() #.pop() later
                    if dp: print var
                    assert var in p.QuantPfx[g][-1].qvars
                    if dp: print "Useful to split"

                    g_new = p.NewGvar()
                    if dp: print p.RepTable[g]
                    p.RepTable[g_new] \
                        = tuple([p.RepTable[g][0]] \
                                + [child \
                                   for child in p.RepTable[g][1:] \
                                   if abs(child) in Vars2gates[var]])
                    if dp: print p.RepTable[g_new]
                    p.RepTable[g] = tuple(\
                        [p.RepTable[g][0]] \
                        + [child for child in p.RepTable[g][1:] \
                           if abs(child) not in Vars2gates[var]] \
                        + [g_new])
                    if dp: print p.RepTable[g]

                    # Handle below (more elegant methinks)
                    if dp: print "Moving quantifier for ", var
                    origQB = p.QuantPfx[g][-1]
                    #origQB.qvars.remove(var)
                    #p.QuantPfx[p.NumVars] = [QuantBlk(origQB.qtype, [var])]
                    #for child_idx in xrange(len(p.RepTable[g])-1, 0,-1):
                    #    child = p.RepTable[g][child_idx]
                    #    if abs(child) in Vars2gates[var]:
                    #        p.RepTable[p.NumVars].append(child)
                    #        del p.RepTable[g][child_idx] #slow
                    #p.RepTable[g].append(p.NumVars)

                    separatedgates = Vars2gates[var]
                    affectedvars = set()
                    for sg in separatedgates:
                        affectedvars |= GateDesc[sg] #might get too big. Opt?
                    if dp: print affectedvars

                    sg_vars_subsumed = []
                    sg_vars_notsubsumed = []
                    for sg_var in Vars2gates:
                        if sg_var not in affectedvars: continue
                        if set(Vars2gates[sg_var]) <= set(separatedgates):
                            sg_vars_subsumed.append(sg_var)
                        else:
                            sg_vars_notsubsumed.append(sg_var)
                            
                    if dp: print sg_vars_subsumed
                    if dp: print sg_vars_notsubsumed
                            
                    origQB.qvars = list(
                        set(origQB.qvars) - set(sg_vars_subsumed))
                    p.QuantPfx[g_new] = [
                        QuantBlk(origQB.qtype, sg_vars_subsumed)]
                    if dp: print p.QuantPfx[g_new]
                    for sg_var in sg_vars_subsumed + sg_vars_notsubsumed:
                        count = len(Vars2gates[sg_var])
                        if count > 4: VarsByLen[5].remove(sg_var)
                        else: VarsByLen[count].remove(sg_var)
                        del Vars2gates[sg_var]
    FindVarLevs(p)
    FindParents(p)


    # The following works well for dmeX_X instances
    GateDesc = {}
    getGateDesc(p.OutGlit)
    MoveQ = {}

    for (b, qp) in p.QuantPfx.items():
        if len(qp) != 1: continue  # only one quantifier block in the prefix.
        if len(qp[0].qvars) != 1: continue
        u = qp[0].qvars[0]
        if (u not in GateDesc[b]): continue
        if (len(GateDesc[b]) != 2): continue
        e = list(GateDesc[b] - set([u]))[0]
        assert(GateDesc[b] == set([e,u]))
        if (set(p.VarPars[e]) != set(p.RepTable[b][1:])): continue
        MoveQ[e] = b

    for (b, qp) in p.QuantPfx.items():
        for qb in qp:
            qb.qvars = [x for x in qb.qvars if (x not in MoveQ)]
    for (e, b) in MoveQ.items():
        p.QuantPfx[b].insert(0, QuantBlk(p.QtypeFromVar[e], [e]))
...
def main():
    ...
    if len(argv) < 2 or (argv[1] in ["-h", "--help", "-help"]):
        ...
        print "\n".join([
            ...
            " '-unprenex 1':    Tries to push quantifiers inward.",
            ...
            " '-write-json':    Write a JSON-encoded circuit file.",
            ])
        sys.exit(1)
    ...
    glo.NoVarAlias = True
    glo.ConvOr = False
    glo.UnprenexSplit = False
    glo.TryUnprenex = False
    glo.MaxGateSize = 40000
    glo.NoPrintPreprocTime = False
    glo.TryBloqqer = False
    ...
    while (i < len(argv)):
        ...
        if (False): pass
        ...
        elif (sArg=='-write-json'):     glo.OutputFmt = 'jqbf'
        ...
        elif (sArg=='-unprenex-split'):  # Allow splitting up gates
            i+=1
            glo.UnprenexSplit = bool(int(argv[i]))
        elif (sArg=='-unprenex'):
            i+=1
            glo.TryUnprenex = bool(int(argv[i]))
            ...
            i+=1
            bloqqer = re.match('[-A-Za-z0-9_./]*', argv[i]).group(0)
            glo.TryBloqqer = which_exe(bloqqer)
            if (glo.TryBloqqer is None):
                qdie("Error: bloqqer executable '" + bloqqer + "' not found.")
        elif (sArg=='-o'):
            i+=1
            OutFilename = argv[i]
    ...
    def TransmogrifyProb(p):
        ...
        if (PrStage): 
            if p.NumPushed == -1:
                pass
                #sys.stderr.write("Skipped unprenexing ('-unprenex' option not specified).\n")
            else:
                sys.stderr.write("Done unprenexing (%i), %.4f sec\n" % (p.NumPushed, elapsed_time()))
        if not glo.NoRenum: 
            RenumExprs(p)
    ...
    def PrintProb(p, outf):
        NumOrigVars = p.NumOrigVars
        if (not glo.NoRenum):
            factor = NumOrigVars / (p.NumVars/2)
            if PrStage:
                sys.stderr.write((
                    "Input variables reduced from %5i to %4i (%ix), NumClauses=%i\n") % \
                    (NumOrigVars, p.NumVars/2, factor, sum(len(v)+1 for v in p.RepTable.values())))
    ...
    def PrintJson(p, outf):
        import json
        from OrderedDict import OrderedDict
        NumOrigVars = p.NumOrigVars
        def PrintParam(key, val, raw=False):
            if not raw:
                val = json.dumps(val)
            outf.write('"%s": %s,\n' % (key, val))

        outf.write("{\n")
        PrintParam('format','CktQBF')
        PrintParam('format_version', '2011-11-17')
        PrintParam("LastInputVar", p.NumVars)
        PrintParam("LastGateVar", max(p.RepTable.keys()))
        PrintParam("OutputGateLit", p.OutGlit)
        PrintParam("PreprocTimeMilli", int(elapsed_time() * 1000))
        
        if WriteOldMap:
            PrintParam("MapNewToOld", p.MapNewToOld.items())
            PrintParam("Subsumed", sorted(p.Subsumed))

        CondensePfx(p)
        
        if (len(p.QuantPfx) != 1):
            die("Only prenex instances are supported.")
        [(qgate, blks)] = SortedQuantPfx(p)
        if (qgate != p.OutGlit):
            die("Only prenex instances are supported.")
        blks = list(OrderedDict([("qtype", b.qtype.lower()), ("qvars", b.qvars)]) for b in blks)
        outf.write('"quantifier_prefix": [\n')
        outf.write(",\n".join("  %s" % (json.dumps(blk),) for blk in blks))
        outf.write('\n],\n')
        ...
        def iter_gates():
            ...
            for (RepVar, Expr) in reversed(sorted(p.RepTable.items())):  #, key=(lambda x: p.MapNewToOld[x])
                ...
                if (CurLev != NewLev):
                    CurLev = NewLev
                    #outf.write("#NESTING LEVEL %i\n" % CurLev)
                OccurMsg = ""
                yield '  [%i, "=", ["%s", %s]]' % (RepVar, Expr[0], sorted(Expr[1:], key=abs))
                #outf.write("%i = %s(%s)%s\n" % 
                #    (RepVar, Expr[0], ", ".join([str(lit) for lit in sorted(Expr[1:], key=abs)]), OccurMsg))
        outf.write('"matrix": [\n' + ",\n".join(iter_gates()) + "\n]\n")
        outf.write("}\n")
