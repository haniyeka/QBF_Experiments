 ...
 *
 * \return the pair <y0, y1> (if one of these variables did not need to be created it is replaced by 0).
 * \todo Use a SAT call to test for an implication between \f$x\f$ and \f$y\f$.
 * \todo Check if we can add blocked implications to reduce the number of variable copies.
 */
std::pair<Variable, Variable> Formula::elimDependency(const Variable x, const Variable y)
{
    ...
    if (x_implies_y && x_implies_ny) {
        // x => y and x => ~y means that ~x is unit (and actually the formula unsatisfiable as x is universal).
        pushUnit(var2lit(x, true));
        return std::make_pair(0u, 0u);
    }

    if (nx_implies_y && nx_implies_ny) {
        // ~x => y and ~x => ~y means that x is unit (and actually the formula unsatisfiable as x is universal).
        pushUnit(var2lit(x, false));
        return std::make_pair(0u, 0u);
    }

    if (x_implies_y && nx_implies_y) {
        // x => y && ~x => y means that y is always implied.
        pushUnit(var2lit(y, false));
        return std::make_pair(0u, 0u);
    }

    if (x_implies_ny && nx_implies_ny) {
        // x => ~y && ~x => ~y means that ~y is always implied.
        pushUnit(var2lit(y, true));
        return std::make_pair(0u, 0u);
    }
...
//TODO: Gate with constant input
        g._input_literals = { var2lit(x, false), 1, var2lit(y0, false) }; // select, in1, in0
        ...
        // x -> ~y. That means, y1 = 0 and the equation simplifies to y = ~x*y0, i.e., an AND-gate
        // Clauses: (~x, ~y) (y0, ~y) (x, ~y0, y)
        const Variable y0 = copyVar(y);
        ...
        // ~x -> ~y. That means, y0 = 0 and the quation simplifies to y = x*y1, i.e., an AND-gate
        // Clauses: (x, ~y) (y1, ~y) (~x, ~y1, y)
        const Variable y1 = copyVar(y);
        ...
        if (c_nr2 >= 0) g._encoding_clauses.push_back(c_nr2);
        if (c_nr3 >= 0) g._encoding_clauses.push_back(c_nr3);
        if (c_nr4 >= 0) g._encoding_clauses.push_back(c_nr4);
        _gates.addGate(std::move(g));
