...
public:
    ...
    */
     ...
     */
    void setSettings(const Settings& formula_setting);
    ...
    void setVerbosity(el::base::type::VerboseLevel val) noexcept;
     ...
     */
    void elimEVar(Variable var, std::set<Variable>* recalc_vars = nullptr);
...
private:
   ...
   //@}
    ...
    //@{
    /// \nameUnit propagation and pure literals
    void pushUnit(Literal lit, bool is_pure = false);
    ...
    bool checkBlockedLit(Literal lit);
...
public:
    ...
    int computeExpansionCosts2(Literal ulit, const std::set<Variable>& pseudo_deps);

private:
    void markTransitiveUnits(std::stack<Literal>& units, std::vector<bool>& marked) const;

    // Subsumption checks
    std::size_t isBackwardSubsuming(const Clause& short_clause, int c_nr = -1);
    template <typename Container>
    ...
    std::array<std::size_t, static_cast<unsigned int>(Statistics::_last_)> _statistics; ///< Statistics for the different operations
    std::array<Timer, static_cast<unsigned int>(WhichTimer::_last_)> _timers;   ///< Timers for the different operations
};
