...
inline std::size_t Formula::getLevel(Variable var) const noexcept
{
    ...
    val_assert(_qbf_prefix != nullptr);
    val_assert(minVarIndex() <= var && var <= maxVarIndex());
...
inline std::int32_t Formula::varAssignment(const Variable var) const noexcept
{
   ...
   */
inline void Formula::setSettings(const Settings& formula_setting)
{
    _settings = formula_setting;
    el::Loggers::setVerboseLevel(_settings.verbosity);
}

inline Settings Formula::getSettings() const
{
    return _settings;
}
...
inline void Formula::setVerbosity(const el::base::type::VerboseLevel val) noexcept
{
...
inline void Formula::setForceDQBF(const bool val) noexcept
{
    _settings.force_dqbf = val;

    if (_settings.force_dqbf && _prefix->type() == PrefixType::QBF) {
        VLOG(2) << "Switching to DQBF prefix representation as requested by user.";
        _dqbf_prefix = _qbf_prefix->convertToDQBF();
        delete _qbf_prefix;
        _qbf_prefix = nullptr;
        _prefix = _dqbf_prefix;
...
inline void Formula::printSettings() const
{
    LOG(INFO) << _settings << std::endl;
}
