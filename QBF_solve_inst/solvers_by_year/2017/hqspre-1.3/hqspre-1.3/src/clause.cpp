...
bool Clause::subsetOf(const Container& other, const std::uint64_t other_signature) const
{
...
bool Clause::checkConsistency() const
{
    for (const Literal lit: _literals) {
        if (lit < 2) {
            LOG(ERROR) << "Invalid literal in clause: " << lit << std::endl;
            return false;
        }
    }

    if (!std::is_sorted(_literals.cbegin(), _literals.cend())) {
        LOG(ERROR) << "Clause is not sorted: " << *this << std::endl;
        return false;
    }

    if (!is_unique(_literals.cbegin(), _literals.cend())) {
        LOG(ERROR) << "Clause contains duplicate literals: " << *this << std::endl;
        return false;
    }

    if (isTautology()) {
        LOG(ERROR) << "Clause is a tautology: " << *this << std::endl;
        return false;
    }
