...
#  define val_assert(x) \
        do { if (!(x)) { VALGRIND_PRINTF_BACKTRACE("Assertion failed at %s:%i!\n", __FILE__, __LINE__); assert(x); } } while(0)

#  define val_assert_msg(x, y) \
        do { if (!(x)) { VALGRIND_PRINTF_BACKTRACE("Assertion failed at %s:%i!\nReason: %s\n", __FILE__, __LINE__, y); assert(x); } } while(0)
#else
#  define val_assert(x) \
        assert(x)
...
inline void fast_set_union(const T1& source, T2& target)
{
 ...
 * duplicates, this function checks if \f$A\setminus B\f$ is empty. The result is true iff
 * \f$A\setminus B\f$ is empty.
 *
...
bool set_difference_empty(InputIterator1 a_iter, InputIterator1 a_end,
                          InputIterator2 b_iter, InputIterator2 b_end)
{
    while (a_iter != a_end) {
        if (b_iter == b_end || *a_iter < *b_iter) { return false; }
        else if (*a_iter == *b_iter) { ++a_iter; ++b_iter; }
        else { // *a_iter > *b_iter
            ++b_iter;
        }
    }
...
/**
 * Given two sorted ranges A = [a_iter, a_end)  and B = [b_iter, b_end) without duplicates, 
 * this function checks if \f$A\setminus B\f$ and \f$B\setminus A\f$ are empty. The result is a pair (e_a,e_b) such
 * that e_a = true iff \f$A\setminus B\f$ is empty and e_b = true iff \f$B\setminus A\f$ is empty.
 *
 * \param a_iter Iterator pointing to the first element of the first range
 * \param a_end Iterator pointing behind the last element of the first range
 * \param b_iter Iterator pointing to the first element of the second range
 ...
 */
template <typename InputIterator1, typename InputIterator2>
std::pair<bool, bool> two_sided_difference_empty(InputIterator1 a_iter, InputIterator1 a_end,
                                                 InputIterator2 b_iter, InputIterator2 b_end)
{
    ...
    while (true) {
        if (a_iter == a_end && b_iter == b_end) break;
        else if (a_iter == a_end) { b_minus_a_empty = false; break;}
        else if (b_iter == b_end) { a_minus_b_empty = false; break; }
        else if (*a_iter == *b_iter) { ++a_iter; ++b_iter; }
        else if (*a_iter < *b_iter) { ++a_iter; a_minus_b_empty = false; }
        else if (*a_iter > *b_iter) { ++b_iter; b_minus_a_empty = false; }
...
/**
 * Given two sorted ranges A = [a_iter, a_end) and B = [b_iter, b_end) without duplicates,
 * this function computes \f$A\setminus B\f$ and \f$B\setminus A\f$ and stores them in the sets
 * a_minus_b and b_minus_a, respectively.
 *
 ...
 * \param b_end Iterator pointing behind the last element of the second range
 * \param a_minus_b Iterator to the container in which the elements of \f$A\setminus B\f$ are stored
 * \param b_minus_a Iterator to the container in which the elements of \f$B\setminus A\f$ are stored
 * \return a pair of iterators pointing to the end of the constructed ranges.
 */
template <typename InputIterator1, typename InputIterator2,
          typename OutputIterator1, typename OutputIterator2>
std::pair<OutputIterator1, OutputIterator2>
two_sided_difference(InputIterator1 a_iter, InputIterator1 a_end,
                     InputIterator2 b_iter, InputIterator2 b_end,
 ...
 */
template <typename InputIterator1, typename InputIterator2>
std::size_t set_difference_count(InputIterator1 a_iter, InputIterator1 a_end,
                                  InputIterator2 b_iter, InputIterator2 b_end)
{
    std::size_t a_count = 0;
    while (a_iter != a_end) {
        if (b_iter == b_end)         { ++a_iter; ++a_count; }
        else if (*a_iter == *b_iter) { ++a_iter; ++b_iter;  }
        else if (*a_iter < *b_iter)  { ++a_iter; ++a_count; }
        else                         { ++b_iter; }  // *a_iter > *minus_iter
    }
