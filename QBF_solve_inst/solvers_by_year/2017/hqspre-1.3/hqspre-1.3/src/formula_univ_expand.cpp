...
#include "../libs/antom/solver/antom.h"
 ...
 */
int Formula::computeExpansionCosts2(const Literal ulit, const std::set<Variable>& pseudo_deps)
{
        ...
        while (true) {
            ...
            for (int level = static_cast<int>(_qbf_prefix->getMaxLevel()); level >= 0; --level) {
                ...
                while (!block.empty()) {
                    ...
                    for (Variable uvar: block) {
                        // sstdQuadDep(uvar, true, true, &pseudo_deps);
                        const int cost = computeExpansionCosts2(var2lit(uvar, false), pseudo_deps);
                        pseudo_deps.clear();
						/*
						const int cost2 = computeExpansionCosts2(var2lit(uvar, true), pseudo_deps);
						pseudo_deps.clear();
						std::cout << uvar << " poslit: " << var2lit(uvar,false) << " costs: " << cost << " neglit: " << var2lit(uvar, true) << " costs: " << cost2 << std::endl;
						*/
                    ...
                    ++count;
                    if (static_cast<double>(literals) > 1.5 * old_size) break;
                }
...
bool Formula::applyUniversalExpansionDQBF()
{
    ...
    for (Variable uvar = minVarIndex(); uvar <= maxVarIndex(); ++uvar) {
        if (isUniversal(uvar)) {
            clause[0] = var2lit(uvar, true);
            antom.AddSoftClause(clause);
        }
    }

    // Solve and determine optimal solution:
    std::int32_t optimum = 0;
    const auto result = antom.MaxSolve(optimum);
    if (result != ANTOM_SAT) {
        VLOG(2) << "Antom could not find a solution for universal expansion.";
        return false;
    }
    const std::vector<unsigned int>& model = antom.Model();
    std::set<Variable> to_eliminate;
    for (Variable var = minVarIndex(); var <= maxVarIndex(); ++var) {
        if (isUniversal(var) && isPositive(model[var])) {
            to_eliminate.insert(var);
        }
    }

    VLOG(2) << "Need to eliminate " << to_eliminate.size() << " univ vars out of " << numUVars() << ":" << to_eliminate;
    ...
    while (true) {
        int next_var = -1;
        int min_cost = std::numeric_limits<int>::max();
        for (const Variable uvar: to_eliminate) {
        ...
        if (next_var >= 0 && (to_eliminate.size() <= 3 || min_cost < 0.1 * old_size)) {
            elimUVar(var2lit(next_var));
            to_eliminate.erase(next_var);
            ++count;
            if (_interrupt) break;
            applyResolution();
            if (static_cast<double>(numLiterals()) > 1.1 * old_size) break;
        } else break;
    }
    return count > 0;
}
