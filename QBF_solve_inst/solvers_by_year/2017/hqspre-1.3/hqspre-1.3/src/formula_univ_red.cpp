 ...
 * \todo Use dependency schemes to make universal reduction more powerful (see paper by Bayersdorff et al.)
 */
bool Formula::universalReduction(Clause& clause, int c_nr)
{
    ...
    if (_prefix->type() == PrefixType::QBF) {
        val_assert(_qbf_prefix != nullptr && _dqbf_prefix == nullptr);
        ...
        if (!_removed_lits.empty()) {
            ...
            if (c_nr >= 0) {
                ...
                if (clause.empty()) throw UNSATException("Universal reduction created an empty clause.");
        ...
        // formula is a DQBF
        val_assert(_prefix->type() == PrefixType::DQBF && _dqbf_prefix != nullptr);
