...
void Formula::preprocess()
{
    ...
    bool again = true;
    bool temp_hidden = _settings.hidden;
    ...
    while (again && stat(Statistics::PREPRO_LOOPS) < _settings.max_loops) {
        ...
        // Substitution
        if (_settings.substitution && stat(Statistics::PREPRO_LOOPS) <= _settings.max_substitution_loops && applySubstitution()) {
            again = true;
            fastPreprocess();
            val_assert(checkConsistency());
        }
        ...
        if (stat(Statistics::PREPRO_LOOPS) == 1 ) _settings.hidden = false;

        // Blocked clause and friends (tautology/subsumption) elimination
        val_assert(checkConsistency());
        if ((_settings.bce || _settings.hse) && removeBlockedAndFriends()) {
            val_assert(checkConsistency());
            again = true;
            fastPreprocess();
            if (_interrupt) return;
 ...
 * new units.
 * \param until_fixedpoint If true, then the operations are called until no more changes happen.
 */
void Formula::fastPreprocess(const bool until_fixedpoint)
{
    ...
    } while (changes && until_fixedpoint);

    VLOG_IF(stat(Statistics::UNIT) > old_stat_unit || stat(Statistics::PURE) > old_stat_pure
            || stat(Statistics::EQUIV_LITS) > old_stat_equiv || stat(Statistics::IMPLICATION_CHAINS) > old_stat_impl_chains, 2)
        << __FUNCTION__ << " detected "
        << (stat(Statistics::UNIT) - old_stat_unit) << " units, "
        << (stat(Statistics::PURE) - old_stat_pure) << " pures, "
        << (stat(Statistics::EQUIV_LITS) - old_stat_equiv) << " equivalent literals, and "
        << (stat(Statistics::IMPLICATION_CHAINS) - old_stat_impl_chains) << " implication chains.";
}
 ...
 */
std::size_t Formula::numDependencies() const noexcept
{
    val_assert(_prefix != nullptr);
 ...
 */
bool Formula::dependenciesSubset(const Variable var1, const Variable var2) const
{
    val_assert(_prefix != nullptr);
 ...
 *
 * \note performs subsumption check: if the binary clause already exists, the clause
 * with ID "c_nr" will be removed and the original one will be kept
 */
void Formula::addImplications(const Literal lit1, const Literal lit2, const ClauseID c_nr)
{
    ...
    if (id >= 0) {
        ...
        // due to current datastructure also the original binary clause will be deleted
 ...
 */
int Formula::addClauseToLists(const ClauseID c_nr, const bool check_subsumption)
{
    ...
    for (const Literal lit: clause) {
        LOG_IF(varDeleted(lit2var(lit)), ERROR) << "Variable " << lit2var(lit) << " is deleted in clause " << clause << '\n';
 ...
 */
int Formula::addClause(const Clause& clause)
{
    if (clause.empty()) {
        throw UNSATException("Trying to add an empty clause");
    }
 ...
 * \throw UNSATException if an empty clause is added.
 */
int Formula::addClause(Clause&& clause)
{
    if (clause.empty()) {
        throw UNSATException("Trying to add an empty clause");
    }
 ...
 * \throw UNSATException if an empty clause is added.
 */
int Formula::addClause(Clause::ClauseData&& clause, bool needs_sorting, bool check_subsumption, ClauseStatus status)
{
    if (clause.empty()) {
        throw UNSATException("Trying to add an empty clause");
    }
 ...
 */
void Formula::removeClause(const ClauseID c_nr)
{
 ...
 */
bool Formula::removeLiteral(const ClauseID c_nr, const Literal lit)
{
    val_assert(!clauseDeleted(c_nr));
...
void Formula::reset()
{
    ...
    for (auto& timer: _timers) { timer.reset(); }
    for (auto& val: _statistics) { val = 0; }
}
 ...
 */
std::pair<bool,bool> Formula::hasImplicationTransitive(const Literal start_lit, const Variable target_var) const
{
...
std::vector<bool> Formula::impliedLiterals(const Literal start_lit) const
{
...
{
    val_assert(_prefix != nullptr);
    val_assert(_prefix->type() == PrefixType::QBF || _dqbf_prefix != nullptr);

    if (_prefix->type() == PrefixType::QBF && _qbf_prefix != nullptr && _dqbf_prefix == nullptr) return true;
...
void Formula::convertToQBF()
{
    ...
    if (_prefix->type() == PrefixType::QBF && _qbf_prefix != nullptr) return;
...
/**
 * \brief Counts occurences of all variables and stores it into `_variable_score`
 */
...
/**
 * \brief Counts implications of all variables and stores it into `_variable_score`
 */
 ...
 */
void Formula::printStatistics() const
{
    LOG(INFO) << "STATISTICS ON THE PREPROCESSING PROCESS:\n"
              ...
              << "c num_gates.....................= " << getStatistics(Statistics::GATES_AND) + getStatistics(Statistics::GATES_XOR) + getStatistics(Statistics::GATES_MUX) + getStatistics(Statistics::GATES_SEMANTIC) << '\n'
