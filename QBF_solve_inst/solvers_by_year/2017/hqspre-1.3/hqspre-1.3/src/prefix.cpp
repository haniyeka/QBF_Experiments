...
void DQBFPrefix::intersectDependencies(const Variable var1, const Variable var2)
{
    ...
    std::set_intersection(_dependencies[var1].cbegin(), _dependencies[var1].cend(),
                          _dependencies[var2].cbegin(), _dependencies[var2].cend(),
...
void DQBFPrefix::uniteDependencies(const Variable var1, const Variable var2)
{
    ...
    std::set<Variable> new_deps;
    std::set_union(_dependencies[var1].cbegin(), _dependencies[var1].cend(),
                   _dependencies[var2].cbegin(), _dependencies[var2].cend(),
                   std::inserter(new_deps, new_deps.end()));
    setDependencies(var1, std::move(new_deps));
}
...
bool Prefix::checkConsistency() const
{
    ...
    if (numEVars() != e_vars) {
        LOG(ERROR) << "num_e_vars is not correct (actual: " << numEVars() << ", correct: " << e_vars << std::endl;
        return false;
    }
    if (numUVars() != u_vars) {
        LOG(ERROR) << "num_u_vars is not correct (actual: " << numUVars() << ", correct: " << u_vars << std::endl;
        return false;
    }
...
bool DQBFPrefix::checkConsistency() const
{
    ...
    if (_univ_vars.size() != numUVars()) {
        LOG(ERROR) << "numUVars() = " << numUVars() << " != univ_vars.size() = " << _univ_vars.size() << std::endl;
        return false;
    }
        ...
        if (_in_rmb[var]) {
            ...
            for (Variable univ: _univ_vars) {
                if (_dependencies[univ].find(var) == _dependencies[univ].cend()) {
                    LOG(ERROR) << "RMB-Variable " << var << " does not appear in the dependency list of " << univ << std::endl;
                    return false;
                }
            }
            continue;
 ...
 */
std::size_t DQBFPrefix::numDependencies() const noexcept
{
 ...
 * \brief Returns the cumulated number of dependencies of all existential variables.
 */
std::size_t QBFPrefix::numDependencies() const noexcept
{
    std::size_t result = 0;
    std::size_t univs = 0;
    for (const auto& block: _blocks) {
 ...
 */
std::size_t DQBFPrefix::numDependencies(Variable var) const noexcept
{
    val_assert(minVarIndex() <= var && var <= maxVarIndex());
    val_assert(isExistential(var));

    if (_in_rmb[var]) return _univ_vars.size();
    else return _dependencies[var].size();
}
 ...
 * \brief Returns the number of dependencies of a given existential variable.
 */
std::size_t QBFPrefix::numDependencies(Variable var) const noexcept
{
    val_assert(minVarIndex() <= var && var <= maxVarIndex());
...
bool DQBFPrefix::isQBF() const
{
    for (Variable var = minVarIndex(); var <= maxVarIndex(); ++var) {
        ...
        for (Variable other_var = var + 1; other_var <= maxVarIndex(); ++other_var) {
            ...
            const auto result = two_sided_difference_empty(_dependencies[var].cbegin(),
                                                           _dependencies[var].cend(),
                                                           _dependencies[other_var].cbegin(),
                                                           _dependencies[other_var].cend());
...
QBFPrefix* DQBFPrefix::convertToQBF() const
{
    ...
    std::sort(exist_vars.begin(), exist_vars.end(),
        [this](Variable a, Variable b)
            {
                return _dependencies[a].size() < _dependencies[b].size();
            }
    );
...
void QBFPrefix::setLevel(const Variable var, const std::size_t level)
{
    val_assert(minVarIndex() <= var && var <= maxVarIndex());
    ...
    val_assert(level == _blocks.size() || _blocks[level].empty() || isExistential(var) == isExistential(*(_blocks[level].begin())));
    val_assert(level < _blocks.size() || _blocks.back().empty() || isExistential(*(_blocks.back().begin())) != isExistential(var));
...
#ifndef NDEBUG
    for (auto dep: deps) {
        val_assert(!varDeleted(dep));
        val_assert(isExistential(var) != isExistential(dep));
    }
#endif
 ...
 */
VariableStatus QBFPrefix::getLevelQuantifier(const std::size_t level) const noexcept
{
    if (level >= _blocks.size() || _blocks[level].empty()) return VariableStatus::DELETED;
    else if (isExistential(*(_blocks[level].begin()))) return VariableStatus::EXISTENTIAL;
